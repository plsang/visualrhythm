...
count=0
for f in `ls /net/per900a/raid0/plsang/dataset/MED10_Resized/*.mp4`
do 
	if [ "$count" -ge $1 ] && [ "$count" -lt $2 ]; then
		...
		./visualrhythm 90 0 0 /net/per900a/raid0/plsang/resources/MED10_Resized $fn /net/per900a/raid0/plsang/dataset/MED10_Resized/sbd-t90/$of
	fi
	let count=$count+1;
